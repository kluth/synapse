...
export class SortMuscle {
  static ascending(): Muscle<number[], number[]> {
    return new Muscle('sortAscending', (arr: number[]) => [...arr].sort((a, b) => a - b), { deterministic: true });
  }

  static descending(): Muscle<number[], number[]> {
    return new Muscle('sortDescending', (arr: number[]) => [...arr].sort((a, b) => b - a), { deterministic: true });
  }
  ...
  static byProperty<T>(property: keyof T): Muscle<T[], T[]> {
    return new Muscle('sortByProperty', (arr: T[]) => {
      return [...arr].sort((a, b) => {
