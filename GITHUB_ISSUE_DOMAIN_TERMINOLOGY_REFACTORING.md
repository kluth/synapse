...
**CLI Components**:
- [ ] `CLI` → `CLI` (keep as-is, or use `CommandLineInterface`)
- [ ] `REPL` → `REPL` (keep as-is, or use `InteractiveShell`)
...
**Core Patterns**:
- [ ] `CQRS` → `CQRS` (keep as-is, or use `CommandQuerySeparation`)
...
**Core Components**:
- [ ] `CRDT` → `CRDT` (keep as-is, or use `ConflictFreeReplicatedDataType`)
